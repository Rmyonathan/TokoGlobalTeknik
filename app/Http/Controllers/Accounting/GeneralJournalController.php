...
class GeneralJournalController extends Controller
{
	...
	{
		$query = Journal::withCount('details');
		...
		if ($q !== '') {
			$query->where(function($w) use ($q){
				$w->where('journal_no', 'like', "%{$q}%")
					->orWhere('reference', 'like', "%{$q}%")
					->orWhere('description', 'like', "%{$q}%");
			});
		}
		
		$journals = $query->orderByDesc('created_at')->paginate(20)->appends($request->query());
		return view('accounting.general_journal.index', compact('journals', 'from', 'to', 'q'));
	}
