...
$(document).ready(function() {
    ...
    $('#supplier').on('input', function () {
        ...
        if (keyword.length > 0) {
            console.log('About to send AJAX request to:', window.supplierSearchUrl);
            $.ajax({
    ...
    function updateItemPreview() {
        ...
        tbody.append(`
            <tr>
                ...
                <td>${quantity}</td>
                <td>${panjang > 0 ? panjang + ' m' : '-'}</td> <!-- Display with meters unit -->
        ...
        items.forEach((item, index) => {
            tbody.append(`
                <tr>
                    ...
                    <td class="text-center">${item.panjang > 0 ? item.panjang + ' m' : '-'}</td> <!-- Display with meters unit -->
        ...
        if (confirm('Apakah Anda yakin ingin menyimpan?')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#invoiceSupplier').text(response.supplier_name || response.kode_supplier);
                    ...
                    $('#printInvoiceBtn').off('click').on('click', function() {
                        window.location.href = window.printInvoiceUrl + transactionId;
                    });
