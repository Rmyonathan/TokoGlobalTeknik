...
<div class="container">
    <div class="title-box">
        <h2><i class="fas fa-edit mr-2"></i>Edit Harga Khusus Pelanggan</h2>
    </div>
        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Form Edit Harga Khusus Pelanggan</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('customer-price.update', $customerPrice->id) }}" method="POST" id="customerPriceForm">
                        @csrf
                        @method('PUT')
                            ...
                            <select class="form-control @error('customer_id') is-invalid @enderror" 
                                ...
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" 
                                            {{ (old('customer_id', $customerPrice->customer_id) == $customer->id) ? 'selected' : '' }}>
                            ...
                            <select class="form-control @error('kode_barang_id') is-invalid @enderror" 
                                ...
                                @foreach($kodeBarangs as $barang)
                                    <option value="{{ $barang->id }}" 
                                            ...
                                            {{ (old('kode_barang_id', $customerPrice->kode_barang_id) == $barang->id) ? 'selected' : '' }}>
                        ...
                        <!-- Harga Khusus -->
                        <div class="form-group mb-3">
                            <label for="harga_jual_khusus" class="form-label">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" step="0.01" min="0" 
                                       ...
                                       value="{{ old('harga_khusus', $customerPrice->harga_jual_khusus) }}" required>
                            </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select class="form-control @error('is_active') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('is_active', $customerPrice->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ old('is_active', $customerPrice->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('is_active')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror" 
                                      ...
                                      placeholder="Keterangan tambahan (opsional)">{{ old('keterangan', $customerPrice->keterangan) }}</textarea>
                            @error('keterangan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-row">
                            <div class="col-md-6">
                                <button type="submit" class="btn btn-primary btn-block">
                                    <i class="fas fa-save mr-1"></i> Update Harga Khusus
                                </button>
                            </div>
                            <div class="col-md-6">
