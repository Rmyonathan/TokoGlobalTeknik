...
<div class="container">
    ...
    <form method="POST" action="{{ route('chart-of-accounts.update', $account) }}">
        @csrf
        @method('PUT')
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select name="account_type_id" class="form-select">
                ...
                @foreach($accountTypes as $type)
                    <option value="{{ $type->id }}" @selected(old('account_type_id', $account->account_type_id)==$type->id)>{{ $type->name }}</option>
                @endforeach
            </select>
        ...
        <div class="form-check mb-3">
            <input class="form-check-input" type="checkbox" name="is_active" value="1" @checked(old('is_active', $account->is_active))>
