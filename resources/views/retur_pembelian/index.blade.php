        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-undo mr-2"></i>Retur Pembelian
                    </h3>
                    <div class="card-tools">
                        @can('create retur pembelian')
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped" id="returTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $returPembelian->firstItem() + $index }}</td>
                                    <td>
                                        <strong>{{ $retur->no_retur }}</strong>
                                    </td>
                                    <td>{{ $retur->tanggal->format('d/m/Y') }}</td>
                                    <td>
                                        {{ $retur->supplier->nama_supplier ?? 'N/A' }}
                                        <br><small class="text-muted">{{ $retur->kode_supplier }}</small>
                                    </td>
                    ...
                    <div class="d-flex justify-content-between">
                        <div>
                            Menampilkan {{ $returPembelian->firstItem() }} sampai {{ $returPembelian->lastItem() }} 
                            dari {{ $returPembelian->total() }} data
                        </div>
                        <div>
                            {{ $returPembelian->links() }}
                        </div>
                    </div>
                </div>
            </div>
...
function approveRetur(id) {
    if (confirm('Apakah Anda yakin ingin menyetujui retur pembelian ini?')) {
...
function processRetur(id) {
    if (confirm('Apakah Anda yakin ingin memproses retur pembelian ini? Stok akan disesuaikan.')) {
        const form = document.createElement('form');
        form.method = 'POST';
...
function deleteRetur(id) {
    if (confirm('Apakah Anda yakin ingin menghapus retur pembelian ini?')) {
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = `/retur-pembelian/${id}`;
